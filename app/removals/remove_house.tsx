...
const RemoveHouseInfo = () => {
  const [houseId, setHouseId] = useState('');
  const [adminId, setAdminId] = useState('');
  const [responseMessage, setResponseMessage] = useState('');

  const handleRemoveHouse = async () => {
    if (!houseId.trim() || !adminId.trim()) {
      alert('Please fill in both House ID and Admin ID');
      return;
    }
    ...
    try {
      const response = await fetch('https://owm.onrender.com/admin/remove_house', {
        method: 'DELETE',
        headers: {
    ...
    <ScrollView>
      <View className="w-full bg-yellow-400" style={{ paddingVertical: 40, height: 800 }}>
        ...
        <Text style={{ fontFamily: 'SatoshiItalic', fontSize: 16, marginLeft: 10, marginTop: 10 }}>
          Enter the house ID and your admin ID to remove a house from the system.
        </Text>
        ...
        <View className="mt-8 ml-4">
          ...
          <Pressable onPress={handleRemoveHouse}>
            <Text
              style={{
                fontFamily: 'Satoshi',
                fontSize: 20,
...
export default RemoveHouseInfo;

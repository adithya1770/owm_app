...
const RemoveBillingInfo = () => {
  const [billId, setBillId] = useState('');
  const [adminId, setAdminId] = useState('');
  const [responseMessage, setResponseMessage] = useState('');

  const handleRemoveBilling = async () => {
    if (!billId.trim() || !adminId.trim()) {
      alert('Please fill in both Bill ID and Admin ID');
      return;
    }
    ...
    try {
      const response = await fetch('https://owm.onrender.com/admin/remove_billing', {
        method: 'DELETE',
        headers: {
      ...
      console.error('Error removing billing:', error);
    ...
    <ScrollView>
      <View className="w-full bg-yellow-400" style={{ paddingVertical: 40, height: 800 }}>
        ...
        <Text style={{ fontFamily: 'SatoshiItalic', fontSize: 16, marginLeft: 10, marginTop: 10 }}>
          Enter the bill ID and your admin ID to remove a billing entry from the system.
        </Text>
        ...
        <View className="mt-8 ml-4">
          ...
          <Pressable onPress={handleRemoveBilling}>
            <Text
              style={{
                fontFamily: 'Satoshi',
                fontSize: 20,
...
export default RemoveBillingInfo;
